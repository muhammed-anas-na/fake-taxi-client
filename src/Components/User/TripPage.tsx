...
import {
  addFindCab,
  updateStatus,
} from "../../utils/Redux/Slice/FindCabSlice";
...
export default function TripPage() {
  ...
  useEffect(() => {
    const fetchDriverDetails = async () => {
      try {
        ...
        const pickup_coordinates = await getCoordinatesFn(
          tripData.tripData.pickup_location
        );
    ...
    socket.on("desitination-reached", async (dropoff_time) => {
      ...
      if (tripData.payment == "stripe") {
        const stripe = await loadStripe(
          "pk_test_51Ofb7lSDYoKc4bOt3tO0icPQcW1q1IBBZu4p35fc8mvSMr3mxachNi1SefUUkrQQ98ObCcQ9iWdXokr755TBYEWR00mV7N2ItI"
        );
  ...
  const handleCancleTrip=(reason,selectedOption)=>{
    dispatch(updateStatus("trip-cancelled"));
    dispatch(addFindCab({cancelReason :reason }))
    dispatch(addFindCab({selectedOption:selectedOption}))
    const driverSocketId = tripData.Id
    socket.emit('cancel-trip-by-user' , {driverSocketId , reason , selectedOption});
    navigate(`/trip-cancel/${tripData.tripData._id}`);
  }
  return (
    <div className="bg-hero h-[99vh] bg-no-repeat scrollbar-hide w-full">
      <ToastContainer />
      <Fa
        onClick={() => navigate(-1)}
        name="arrow-left"
              ...
              <div className="h-[80vh] overflow-y-auto scrollbar-hide">
                ...
                <div className="flex flex-row">
                  <div>
                    <div className="flex gap-2 md:my-5">
                      ...
                      <div>
                        ...
                        >
                          {driverDetails
                            ? driverDetails?.vehicle_details?.vehicle_name +
                              "," +
                              driverDetails?.vehicle_details?.vehicle_brand
                            : "Loading..."}
                        </Typography>
                      </div>
                    </div>
                    <h1>
                      {driverDetails ? driverDetails?.phone : "Loading..."}
                    </h1>
                  ...
                  <div className="ml-auto">
                    <h1 className="my-4">
                      {driverDetails
                        ? driverDetails?.vehicle_details?.rc_number
                        : "Loading..."}
                    </h1>
                    <h1 className="md:mt-12">
                      {tripData.tripData.pickup_location}
                    </h1>
                    <h1 className="md:my-2">
                      {tripData.tripData.dropoff_location}
                    </h1>
