...
const Chat = () => {
  const driverSocketId = useSelector((store) => store.findcab.findcabData.Id);
          ...
          {totalMessage.map((value, index) => (
            <div key={index} className={value.type === "send" ? "chat chat-end" : "chat chat-start"}>
              ...
              <div className="chat-header">
                ...
                <time className="text-xs opacity-50">{`${new Date().getHours().toString().padStart(2, '0')}:${new Date().getMinutes().toString().padStart(2, '0')}`}</time>
              </div>
